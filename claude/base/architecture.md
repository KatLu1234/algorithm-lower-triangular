...
| `server/` | ASGI 서버 | `uvicorn[standard]` | `requirements.txt` 고정. 실행: `uvicorn app.main:app` |
| `server/` | 검증 / 모델 | Pydantic v2 | `app/schemas/` |
| `server/` | DB | Supabase | `app/db/supabase.py` |
| `server/` | LLM 호출 | 표준 `urllib` → Upstage Solar | `app/libs/llm_client.py`(단일 진입점). 키 `app/core/config.UPSTAGE_*` |
...
### 3.1 `frontend/`
- **해야 할 일**
  - 사용자 입력 폼(강의 후보·학점·중요도·blackout·쉬는시간 등 선호)을 받아 `server`의 `/api/v1/...` 엔드포인트로 JSON POST.
...
- **해야 할 일**
  - **순수 데이터·텍스트 자료**만 보관 (`.md`, `.txt`, `.json`).
    ...
    - `domain/` — 문제 정의서, 용어집, 시간표 추천 도메인·알고리즘 설명 등 LLM이 참고할 도메인 자료
...
1. **frontend** → `POST /api/v1/timetable/solve` with `{"preference": {…PreferenceVector…}, "top_n": 3, "explain": true}`
2. **server**: `endpoints/timetable.py`
   1. `TimetableRequest`(Pydantic)로 입력 검증 (`preference` = `app/schemas/preferences.PreferenceVector`).
   2. 순수 알고리즘 `libs.timetable.recommend(prefs, building_codes, base_walk_minutes, top_k)` 호출
      → A 가능성 → B 가치 → C 선택. A에서 infeasibility 검출 시 B·C를 건너뛰고 `InfeasibilityReport` 반환.
   3. `explain=True`면 `libs.llm_context`가 `claude/llm-include/prompts/*.md`(예: `preference_extract.md`)와
      payload를 합쳐 메시지 배열을 조립.
   4. `libs.llm_client.complete(messages)` → LLM 응답 문자열 (현재 MVP: Upstage 키 미설정 시 `explanation=null`).
   5. `TimetableResponse(selection=…, infeasibility=…, explanation=…)`로 직렬화해 반환.
3. **frontend**: 응답을 받아 상위 N개 시간표 + 점수 분해 + 포함/배제 사유(+설명)를 화면에 표시.
...
- 에러 응답 표준:
  ```json
  { "detail": "사람이 읽을 수 있는 에러 메시지", "code": "VALIDATION_ERROR" }
  ```
...
└── llm-include/
    ...
    ├── progress.md            # 기술 스택 변경·별도 지침 이력
    └── prompts/
        └── preference_extract.md   # 자연어 → PreferenceVector delta 추출 프롬프트

app/libs/
├── timetable.py               # 루트 recommend() — A→B→C 오케스트레이션
├── feasibility.py             # A 가능성 (풀 정제·충돌 행렬·가지치기)
├── valuation.py               # B 가치 (v(c)·top-K 백트래킹)
├── selection.py               # C 선택 (정렬·LCS·포함/배제 사유)
├── floyd_warshall.py · activity_selection.py · knapsack.py · merge_sort.py · lcs.py · binary_search.py  # 단위 알고리즘
├── buildings_loader.py · timeroom_parser.py   # 건물·교시 CSV 로더
├── llm_context.py             # 프롬프트 조립기 (llm-include 읽음)
└── llm_client.py              # LLM 단일 진입점 (urllib → Upstage)
```
